// =====================================================
// 테스트/결과별 시각 정보 매핑
// =====================================================

export type TestIllustration = {
  /** 카드 썸네일 대표 이모지 */
  emoji: string;
  /** 배경 그라데이션 (inline CSS linear-gradient) */
  gradient: string;
  /** 포인트 컬러 hex */
  accentColor: string;
  /** 배경 연한 컬러 hex */
  bgColor: string;
  /** 카드 썸네일용 장식 이모지 목록 */
  decorEmojis: string[];
};

export type ResultIllustration = {
  /** 결과 대표 이모지 (크게 표시) */
  emoji: string;
  /** 결과 카드 헤더 그라데이션 (inline CSS) */
  gradient: string;
  /** 결과 accent 색상 */
  accentColor: string;
  /** 결과 배경 연한 색상 */
  bgColor: string;
};

// ──────────────────────────────
// 카테고리 기본 비주얼
// ──────────────────────────────
const CATEGORY_VISUALS: Record<string, TestIllustration> = {
  yeonae: {
    emoji: "💕",
    gradient: "linear-gradient(135deg, #EC4899 0%, #F472B6 50%, #FB7185 100%)",
    accentColor: "#EC4899",
    bgColor: "#FDF2F8",
    decorEmojis: ["💔", "✨", "🌸"],
  },
  sseom: {
    emoji: "💘",
    gradient: "linear-gradient(135deg, #F472B6 0%, #E879F9 50%, #FB7185 100%)",
    accentColor: "#F472B6",
    bgColor: "#FFF0F6",
    decorEmojis: ["💌", "🌷", "✨"],
  },
  ibyeol: {
    emoji: "💔",
    gradient: "linear-gradient(135deg, #8B5CF6 0%, #A78BFA 50%, #818CF8 100%)",
    accentColor: "#8B5CF6",
    bgColor: "#F5F3FF",
    decorEmojis: ["🌧️", "😢", "🍂"],
  },
  gyeolhon: {
    emoji: "💍",
    gradient: "linear-gradient(135deg, #D946EF 0%, #F472B6 50%, #A78BFA 100%)",
    accentColor: "#D946EF",
    bgColor: "#FDF4FF",
    decorEmojis: ["💐", "🥂", "✨"],
  },
  "ingangwan-gye": {
    emoji: "🤝",
    gradient: "linear-gradient(135deg, #6366F1 0%, #60A5FA 50%, #818CF8 100%)",
    accentColor: "#6366F1",
    bgColor: "#EEF2FF",
    decorEmojis: ["👥", "💬", "🔗"],
  },
  chingu: {
    emoji: "👫",
    gradient: "linear-gradient(135deg, #22C55E 0%, #34D399 50%, #2DD4BF 100%)",
    accentColor: "#22C55E",
    bgColor: "#F0FDF4",
    decorEmojis: ["🫂", "🎉", "🌈"],
  },
  jikjang: {
    emoji: "💼",
    gradient: "linear-gradient(135deg, #3B82F6 0%, #38BDF8 50%, #22D3EE 100%)",
    accentColor: "#3B82F6",
    bgColor: "#EFF6FF",
    decorEmojis: ["📊", "💻", "☕"],
  },
  "don-sobi": {
    emoji: "💰",
    gradient: "linear-gradient(135deg, #F97316 0%, #FBBF24 50%, #FCD34D 100%)",
    accentColor: "#F97316",
    bgColor: "#FFF7ED",
    decorEmojis: ["💸", "📈", "🏦"],
  },
  "seonggong-saup": {
    emoji: "🚀",
    gradient: "linear-gradient(135deg, #EAB308 0%, #FBBF24 50%, #FB923C 100%)",
    accentColor: "#EAB308",
    bgColor: "#FEFCE8",
    decorEmojis: ["🏆", "📊", "⚡"],
  },
  seonggyek: {
    emoji: "🧠",
    gradient: "linear-gradient(135deg, #7C3AED 0%, #8B5CF6 50%, #6366F1 100%)",
    accentColor: "#7C3AED",
    bgColor: "#F5F3FF",
    decorEmojis: ["⚡", "🌀", "✨"],
  },
  jajungam: {
    emoji: "🌱",
    gradient: "linear-gradient(135deg, #10B981 0%, #34D399 50%, #2DD4BF 100%)",
    accentColor: "#10B981",
    bgColor: "#ECFDF5",
    decorEmojis: ["🌿", "☀️", "💚"],
  },
  seuteuleseu: {
    emoji: "😤",
    gradient: "linear-gradient(135deg, #EF4444 0%, #F87171 50%, #FB923C 100%)",
    accentColor: "#EF4444",
    bgColor: "#FEF2F2",
    decorEmojis: ["⚡", "😰", "🔥"],
  },
  muheuishik: {
    emoji: "🔮",
    gradient: "linear-gradient(135deg, #1D4ED8 0%, #6366F1 50%, #8B5CF6 100%)",
    accentColor: "#1D4ED8",
    bgColor: "#EFF6FF",
    decorEmojis: ["🌙", "⭐", "💫"],
  },
  gajok: {
    emoji: "🏠",
    gradient: "linear-gradient(135deg, #059669 0%, #10B981 50%, #34D399 100%)",
    accentColor: "#059669",
    bgColor: "#ECFDF5",
    decorEmojis: ["👨‍👩‍👧‍👦", "❤️", "🌳"],
  },
  "sosyal-imiji": {
    emoji: "🪞",
    gradient: "linear-gradient(135deg, #0891B2 0%, #22D3EE 50%, #60A5FA 100%)",
    accentColor: "#0891B2",
    bgColor: "#ECFEFF",
    decorEmojis: ["✨", "👁️", "💎"],
  },
  chihyang: {
    emoji: "✨",
    gradient: "linear-gradient(135deg, #BE185D 0%, #EC4899 50%, #C026D3 100%)",
    accentColor: "#BE185D",
    bgColor: "#FDF2F8",
    decorEmojis: ["🎨", "🎵", "☕"],
  },
};

// ──────────────────────────────
// 개별 테스트 슬러그별 오버라이드
// ──────────────────────────────
const TEST_OVERRIDES: Partial<Record<string, TestIllustration>> = {
  "yeonae-gojang-paeteon": {
    emoji: "💔",
    gradient: "linear-gradient(135deg, #F43F5E 0%, #EC4899 50%, #E879F9 100%)",
    accentColor: "#F43F5E",
    bgColor: "#FFF1F2",
    decorEmojis: ["🔄", "😔", "💭"],
  },
  "kkeullim-yuhyeong": {
    emoji: "💫",
    gradient: "linear-gradient(135deg, #EC4899 0%, #F472B6 50%, #FBA04B 100%)",
    accentColor: "#EC4899",
    bgColor: "#FDF2F8",
    decorEmojis: ["👀", "💕", "✨"],
  },
  "jeon-aein-mot-nitneum": {
    emoji: "👤",
    gradient: "linear-gradient(135deg, #8B5CF6 0%, #A78BFA 50%, #F472B6 100%)",
    accentColor: "#8B5CF6",
    bgColor: "#F5F3FF",
    decorEmojis: ["💭", "🔁", "💔"],
  },
  "jipcheok-button": {
    emoji: "🔒",
    gradient: "linear-gradient(135deg, #7C3AED 0%, #8B5CF6 50%, #6366F1 100%)",
    accentColor: "#7C3AED",
    bgColor: "#F5F3FF",
    decorEmojis: ["📍", "⚓", "🧲"],
  },
  "hoepisseonghyang-testeut": {
    emoji: "🚪",
    gradient: "linear-gradient(135deg, #0EA5E9 0%, #38BDF8 50%, #818CF8 100%)",
    accentColor: "#0EA5E9",
    bgColor: "#F0F9FF",
    decorEmojis: ["🏃", "🌬️", "💨"],
  },
  "jiltu-bangshik-testeut": {
    emoji: "👁️",
    gradient: "linear-gradient(135deg, #F59E0B 0%, #FB923C 50%, #EF4444 100%)",
    accentColor: "#F59E0B",
    bgColor: "#FFFBEB",
    decorEmojis: ["⚡", "🔥", "😤"],
  },
  "sohnjeol-dang-ha-neun-iyu": {
    emoji: "✂️",
    gradient: "linear-gradient(135deg, #64748B 0%, #94A3B8 50%, #CBD5E1 100%)",
    accentColor: "#64748B",
    bgColor: "#F8FAFC",
    decorEmojis: ["👤", "💬", "🚫"],
  },
  "don-mot-moneun-iyu": {
    emoji: "💸",
    gradient: "linear-gradient(135deg, #22C55E 0%, #34D399 50%, #2DD4BF 100%)",
    accentColor: "#22C55E",
    bgColor: "#F0FDF4",
    decorEmojis: ["💰", "📉", "🏦"],
  },
  "tongjang-sae-neun-paeteon": {
    emoji: "🏦",
    gradient: "linear-gradient(135deg, #14B8A6 0%, #22D3EE 50%, #38BDF8 100%)",
    accentColor: "#14B8A6",
    bgColor: "#F0FDFA",
    decorEmojis: ["💳", "📊", "💴"],
  },
  "nae-seonggyek-wiheom-bubun": {
    emoji: "⚡",
    gradient: "linear-gradient(135deg, #7C3AED 0%, #A855F7 50%, #E879F9 100%)",
    accentColor: "#7C3AED",
    bgColor: "#F5F3FF",
    decorEmojis: ["🌩️", "🔥", "⚠️"],
  },
  "naega-muneojinan-sungan": {
    emoji: "🌊",
    gradient: "linear-gradient(135deg, #2563EB 0%, #6366F1 50%, #8B5CF6 100%)",
    accentColor: "#2563EB",
    bgColor: "#EFF6FF",
    decorEmojis: ["💧", "🌧️", "😞"],
  },
  "insheng-kkoineum-paeteon": {
    emoji: "🌀",
    gradient: "linear-gradient(135deg, #4F46E5 0%, #7C3AED 50%, #8B5CF6 100%)",
    accentColor: "#4F46E5",
    bgColor: "#EEF2FF",
    decorEmojis: ["🔄", "⛓️", "🗝️"],
  },
};

// ──────────────────────────────
// 전생 직업별 비주얼
// ──────────────────────────────
export type PastLifeJobVisual = {
  emoji: string;
  secondaryEmojis: string[];
  label: string;
  gradient: string;
  accentColor: string;
  bgColor: string;
};

export const PAST_LIFE_JOB_VISUALS: Record<string, PastLifeJobVisual> = {
  학자: {
    emoji: "📜",
    secondaryEmojis: ["📚", "🖊️", "🕯️"],
    label: "학자",
    gradient: "linear-gradient(135deg, #92400E 0%, #D97706 50%, #F59E0B 100%)",
    accentColor: "#92400E",
    bgColor: "#FFFBEB",
  },
  "궁중 화가": {
    emoji: "🎨",
    secondaryEmojis: ["🖌️", "🖼️", "✨"],
    label: "궁중 화가",
    gradient: "linear-gradient(135deg, #1D4ED8 0%, #6366F1 50%, #8B5CF6 100%)",
    accentColor: "#1D4ED8",
    bgColor: "#EFF6FF",
  },
  상인: {
    emoji: "⚖️",
    secondaryEmojis: ["💰", "📦", "🗺️"],
    label: "상인",
    gradient: "linear-gradient(135deg, #D97706 0%, #F59E0B 50%, #FB923C 100%)",
    accentColor: "#D97706",
    bgColor: "#FFFBEB",
  },
  검객: {
    emoji: "⚔️",
    secondaryEmojis: ["🛡️", "🌸", "🗡️"],
    label: "검객",
    gradient: "linear-gradient(135deg, #374151 0%, #4B5563 50%, #6B7280 100%)",
    accentColor: "#374151",
    bgColor: "#F9FAFB",
  },
  점술사: {
    emoji: "🔮",
    secondaryEmojis: ["⭐", "🌙", "✨"],
    label: "점술사",
    gradient: "linear-gradient(135deg, #4338CA 0%, #6366F1 50%, #8B5CF6 100%)",
    accentColor: "#4338CA",
    bgColor: "#EEF2FF",
  },
  의원: {
    emoji: "⚕️",
    secondaryEmojis: ["🌿", "💊", "🕯️"],
    label: "의원",
    gradient: "linear-gradient(135deg, #059669 0%, #10B981 50%, #34D399 100%)",
    accentColor: "#059669",
    bgColor: "#ECFDF5",
  },
  시인: {
    emoji: "🪶",
    secondaryEmojis: ["🌸", "📝", "🌙"],
    label: "시인",
    gradient: "linear-gradient(135deg, #F43F5E 0%, #EC4899 50%, #E879F9 100%)",
    accentColor: "#F43F5E",
    bgColor: "#FFF1F2",
  },
  귀족: {
    emoji: "👑",
    secondaryEmojis: ["💎", "🏰", "🌹"],
    label: "귀족",
    gradient: "linear-gradient(135deg, #B45309 0%, #D97706 50%, #FB923C 100%)",
    accentColor: "#B45309",
    bgColor: "#FFFBEB",
  },
  농부: {
    emoji: "🌾",
    secondaryEmojis: ["🌱", "☀️", "🪵"],
    label: "농부",
    gradient: "linear-gradient(135deg, #15803D 0%, #22C55E 50%, #84CC16 100%)",
    accentColor: "#15803D",
    bgColor: "#F0FDF4",
  },
  장군: {
    emoji: "🏹",
    secondaryEmojis: ["⚔️", "🛡️", "🚩"],
    label: "장군",
    gradient: "linear-gradient(135deg, #B91C1C 0%, #DC2626 50%, #F97316 100%)",
    accentColor: "#B91C1C",
    bgColor: "#FEF2F2",
  },
  무희: {
    emoji: "🎭",
    secondaryEmojis: ["🌸", "🎶", "💃"],
    label: "무희",
    gradient: "linear-gradient(135deg, #C026D3 0%, #EC4899 50%, #F472B6 100%)",
    accentColor: "#C026D3",
    bgColor: "#FDF4FF",
  },
  도공: {
    emoji: "🏺",
    secondaryEmojis: ["🔥", "🪣", "🌍"],
    label: "도공",
    gradient: "linear-gradient(135deg, #EA580C 0%, #F97316 50%, #FBBF24 100%)",
    accentColor: "#EA580C",
    bgColor: "#FFF7ED",
  },
};

/** 전생 결과 텍스트에서 직업 키워드를 찾아 시각 정보 반환 */
export function getPastLifeJobVisual(resultText: string): PastLifeJobVisual | null {
  for (const [key, visual] of Object.entries(PAST_LIFE_JOB_VISUALS)) {
    if (resultText.includes(key)) return visual;
  }
  return null;
}

// ──────────────────────────────
// 공개 API
// ──────────────────────────────

/** 테스트 슬러그로 카드 썸네일 비주얼 반환 */
export function getTestIllustration(slug: string, categorySlug: string): TestIllustration {
  return (
    TEST_OVERRIDES[slug] ??
    CATEGORY_VISUALS[categorySlug] ?? {
      emoji: "🧠",
      gradient: "linear-gradient(135deg, #7C3AED 0%, #8B5CF6 50%, #6366F1 100%)",
      accentColor: "#7C3AED",
      bgColor: "#F5F3FF",
      decorEmojis: ["✨", "⚡", "💫"],
    }
  );
}

/** 결과 카드 헤더 비주얼 반환 (카테고리 기반) */
export function getResultIllustration(categorySlug: string): ResultIllustration {
  const base = CATEGORY_VISUALS[categorySlug];
  if (!base) {
    return {
      emoji: "🧠",
      gradient: "linear-gradient(135deg, #7C3AED 0%, #8B5CF6 50%, #EC4899 100%)",
      accentColor: "#7C3AED",
      bgColor: "#F5F3FF",
    };
  }
  return {
    emoji: base.emoji,
    gradient: base.gradient,
    accentColor: base.accentColor,
    bgColor: base.bgColor,
  };
}


