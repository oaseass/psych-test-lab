...
import { getPlayResult, getPlayedSlugs } from "@/lib/test-engine/storage";
...
export default function ResultPageClient({ params }: Props) {
...
function RecommendedTests({ currentSlug, categorySlug }: { currentSlug: string; categorySlug: string }) {
  ...
  const playedSlugs = getPlayedSlugs();
  const recommended = getRecommendations({ currentSlug, categorySlug, playedSlugs, allTests });
